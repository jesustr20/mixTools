...
_SKELETON_PROMPT = (
    ...
    "- LISTAS Y BLANCOS (mejoras opcionales, si el contenido lo pide):\n"
    "  * LISTAS: si hay párrafos consecutivos que empiezan con un marcador de "
    "enumeración — p. ej. (i), (ii), (iii) o 1., 2., 3. — agrupalos en un <ol> con "
    "un <li> por ítem. CRÍTICO: el marcador original debe quedar como texto literal "
    "DENTRO del <li> (ej. <li>(i) Vinculado…</li>); NUNCA lo elimines para que lo "
    "genere CSS (list-style, counter(), etc.) — si el marcador no queda literal, la "
    "verificación de fidelidad rechaza el resultado. Para que no aparezca doble, no "
    "agregues numeración CSS (podés usar style=\"list-style: none;\").\n"
    "  * LISTAS IMPLÍCITAS (SIN marcador escrito — caso DISTINTO al de arriba): "
    "si hay 2 o más párrafos consecutivos que forman una serie paralela — cada uno "
    "(menos el último) termina en punto y coma, siguiendo una oración introductoria "
    "que termina en dos puntos (\":\") — agrupalos en una lista: <ol> con numeración "
    "generada por CSS, o <ul> con viñetas. Como el texto original NO tiene ningún "
    "marcador literal, acá SÍ es seguro usar numeración/viñetas por CSS (list-style): "
    "no estás borrando ningún carácter. NO agregues marcadores literales (números o "
    "letras) que no estaban en la entrada.\n"
    "  * BLANCOS: los placeholders entre corchetes, como [●] o [COMPLETAR DATO], "
    "envolvélos en <span style=\"background-color: yellow;\">…</span> para "
    "resaltarlos (sin agregar ni quitar caracteres).\n"
    "- ESTRUCTURA SEMÁNTICA (mejoras opcionales, por patrón, en TODO el documento):\n"
    "  * TÍTULO DEL DOCUMENTO: el primer párrafo en negrita+subrayado y centrado "
    "del inicio envolvélos con class=\"titulo-documento\" (clase genérica — sirve "
    "para contratos, fichas, anexos o cualquier tipo de documento).\n"
    "  * TÍTULOS DE SECCIÓN: cualquier párrafo en negrita que actúe como encabezado "
    "de sección envolvélos con class=\"titulo-seccion\". Detectá la FORMA del "
    "encabezado (p. ej. una palabra en mayúsculas tipo CLÁUSULA, ARTÍCULO, SECCIÓN, "
    "NUMERAL, o el patrón \"TÍTULO — descripción\"), no una lista fija de palabras — "
    "cada documento usa vocabulario distinto.\n"
    "  * LISTAS ENUMERADAS — TODAS LAS APARICIONES: la regla de listas de arriba se "
    "aplica a CADA serie de ítems del documento, no solo a la primera. Repasá TODO "
    "el documento y convertí todas las enumeraciones en <ol>/<li>, sin omitir ninguna.\n"
    "  * SUB-ÍTEMS ANIDADOS: si un marcador de enumeración aparece DENTRO de un ítem "
    "ya enumerado (p. ej. el ítem 3 contiene (i), (ii), (iii)), envolvés cada "
    "sub-ítem con class=\"sub-item\" para indentarlo, manteniendo el marcador literal. "
    "Un <p class=\"sub-item\"> con el marcador literal alcanza — no hace falta un <ol> "
    "anidado.\n"
    "  * BLOQUES DE FIRMA: una línea de guiones bajos seguida de un nombre y una "
    "identificación (DNI, RUC, etc.), normalmente cerca del final, envolvés los "
    "párrafos con class=\"firma-bloque\" para centrarlos. CRÍTICO: NO reemplaces los "
    "guiones bajos literales por un borde CSS ni por ninguna línea generada — los "
    "guiones bajos deben seguir como texto literal; el centrado es solo CSS alrededor "
    "del texto existente.\n"
    "  * Si usás estas clases nuevas (.titulo-documento, .titulo-seccion, .sub-item, "
    ".firma-bloque), agregá su regla CSS en el <style> (podés agregar reglas nuevas).\n"
    "- PROHIBIDO \"CORREGIR\": nunca corrijas errores tipográficos, puntuación "
    "duplicada, espacios de más ni ningún otro \"arreglo\" no pedido del texto de "
    "origen. Preservalo EXACTAMENTE como está, imperfecciones incluidas — un \"typo\" "
    "aparente NO se toca.\n"
